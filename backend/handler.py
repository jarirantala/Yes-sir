...
def _handle_delete(event, body):
    item_id = body.get('id')
    item_type = body.get('type')
    item_key = body.get('key')
    
    if not item_type:
        return {'statusCode': 400, 'body': json.dumps({'error': 'Missing type for deletion'})}
        
    success = False
    if item_type == 'todo':
        if not item_id:
            return {'statusCode': 400, 'body': json.dumps({'error': 'Missing id for todo deletion'})}
        success = database.delete_todo_item(item_id)
    elif item_type == 'note':
        if not item_id:
            return {'statusCode': 400, 'body': json.dumps({'error': 'Missing id for note deletion'})}
        success = database.delete_note_item(item_id)
    elif item_type == 'keyword':
        target_key = item_key or item_id
        if not target_key:
            return {'statusCode': 400, 'body': json.dumps({'error': 'Missing key for keyword deletion'})}
        success = database.delete_keyword(target_key)
        
    return {
        'statusCode': 200 if success else 404,
        'body': json.dumps({'success': success, 'message': 'Keyword deleted' if item_type == 'keyword' and success else ('Item deleted' if success else 'Item not found')})
    }
...
def _handle_post(event):
    ...
    return _handle_transcript_command(body)

def _handle_transcript_command(body):

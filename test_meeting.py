import urllib.request
import json
import base64
import os

URL = "https://voiceassistantnsxsctbdhw-voice-assistant.functions.fnc.fr-par.scw.cloud/"
TOKEN = "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..t-F_kF4Lh-64nI6i9nZ8aJ6XyO4j-V_5zB5e1e1r7y7_X7c4z2t9tD3t_X8zD5q2c3v4x6p9_L5z7v4c4r5t8e2w9q7_L5z7v4c4r5t8e2w9q7"

def test_meeting_intent():
    # Simulate a transcript that should trigger a MEETING intent
    payload = {
        "transcript": "Schedule a meeting with Jari tomorrow at 10am about the project",
        "timezone": "Europe/Helsinki"
    }
    
    data = json.dumps(payload).encode('utf-8')
    
    print(f"Sending meeting request to {URL}")
    
    req = urllib.request.Request(URL, data=data, method="POST")
    req.add_header("Content-Type", "application/json")
    req.add_header("X-Auth-Token", TOKEN)
    
    try:
        with urllib.request.urlopen(req) as response:
            print(f"Status: {response.getcode()}")
            print(f"Response: {response.read().decode('utf-8')}")
    except urllib.error.HTTPError as e:
        print(f"Error: {e.code} {e.reason}")
        print(f"Body: {e.read().decode('utf-8')}")
    except Exception as e:
        print(f"Failed: {e}")

if __name__ == "__main__":
    test_meeting_intent()
